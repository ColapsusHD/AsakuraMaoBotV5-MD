...
const handler = async (m, {conn, participants, command, usedPrefix, text}) => {
  ...
  if (!global.db.data.settings[conn.user.jid].restrict) throw ` (*_restrict_*) `;
  
  const kicktext = `_${usedPrefix + command} @${global.suittag}_`;
  ...
  try {
    ...
    const userTag = mentionedUser.split('@')[0];
    const exitoso1 = `@${userTag}`;
    const error1 = `@${userTag}`;
    const error2 = `@${userTag}`;
    ...
    if (response[0]?.status === '200') {
      ...
      conn.sendMessage(m.chat, {mentions: [m.sender], contextInfo: {forwardingScore: 999, isForwarded: true}}, {quoted: m});
    }
  } catch (error) {
    conn.sendMessage(m.chat, {mentions: [m.sender], contextInfo: {forwardingScore: 999, isForwarded: true}}, {quoted: m});
  }
